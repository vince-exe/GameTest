...
bool GameSettingsMenu::init(std::shared_ptr<sf::RenderWindow> windowPtr, std::atomic<bool>* closeFlag) {
	...
	while (!this->exitWindow && !this->closeFlag->load()) {
		while (windowPtr->pollEvent(event)) {
			...
			handleMouseButtonPressed(event, windowPtr);
		}
		draw(windowPtr);
	}
...
void GameSettingsMenu::setSprites(std::shared_ptr<sf::RenderWindow> windowPtr) {
	backText.getSprite().setPosition(20.f, windowPtr->getSize().y - backText.getSprite().getGlobalBounds().height - 40);
}

void GameSettingsMenu::draw(std::shared_ptr<sf::RenderWindow> windowPtr) {
	windowPtr->clear();
...
void GameSettingsMenu::handleMouseButtonPressed(sf::Event& event, std::shared_ptr<sf::RenderWindow> windowPtr) {
