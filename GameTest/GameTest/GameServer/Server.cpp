...
Server::Server(unsigned int port, const std::string& ip, unsigned int maxConnections, unsigned int clearUselessThreadsTime) {
	tcp::endpoint endpoint(boost::asio::ip::address::from_string(ip), port);
	m_acceptorPtr = std::make_unique<tcp::acceptor>(m_ioServicePtr, endpoint);
